...
const CREATE_FILENAME = 'results/create-hsm.sql';
const CREATE_HSM_INFO = `INSERT HSM
			namespace: "%s",
			element_name: "%s",
			default_text: "%s",
			status: "%s",
...
const DELETE_FILENAME = 'results/delete-hsm.sql';
...
const UPDATE_FILENAME = 'results/update-hsm.sql';
...
module.exports = {
    ...
    add_hsm_to_be_created(fb_hsm) {
        fs.appendFileSync(CREATE_FILENAME, util.format(CREATE_HSM_INFO,
            fb_hsm.namespace, fb_hsm.element_name, this.plain_string(fb_hsm.default_text), fb_hsm.status,
    ...
    add_hsm_to_be_updated(garuda_hsm, fb_hsm, field) {
        fs.appendFileSync(UPDATE_FILENAME, util.format(UPDATE_HSM_QUERY, HSM_TABLE, field, this.plain_string(fb_hsm[field]), garuda_hsm.id));
        fs.appendFileSync(UPDATE_FILENAME, util.format(UNDO_UPDATE_HSM_QUERY, HSM_TABLE, field, this.plain_string(garuda_hsm[field]), garuda_hsm.id));
